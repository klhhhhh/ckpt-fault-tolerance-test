...
def save_checkpoint(engine, checkpoint_dir, step, mode="sync", executor=None):
    """
    Save checkpoint using DeepSpeed, supporting Sync & Async modes.
    - Sync: engine.save_checkpoint()
    - Async: Save asynchronously using thread pool
    """
...
def load_checkpoint(engine, checkpoint_dir):
    """
    DeepSpeed automatically restores model parameters, optimizer states, and gradients.
    """
...
def train_and_checkpoint(engine, dataloader, checkpoint_dir, checkpoint_interval, mode="sync"):
    """
    During training, each GPU only saves the parameters it is responsible for, supporting Sync & Async modes, and Tensor Core.
    """
...
if __name__ == "__main__":
    ...
    deepspeed_config = {
        ...
        "zero_optimization": {
            "stage": 2,  # ZeRO Stage 2: Only store parameters it is responsible for
